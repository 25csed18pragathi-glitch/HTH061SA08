...
function SimulationControls({ scenario, setScenario, isRunning, startSimulation, pauseSimulation, resetTraffic, simulationSeed, setSimulationSeed }) {
  return <div className="simulation-controls"><div className="button-row"><button className="primary-button" onClick={startSimulation} disabled={isRunning}><Play size={14} /> START</button><button className="secondary-button" onClick={pauseSimulation} disabled={!isRunning}><Pause size={14} /> PAUSE</button><button className="secondary-button" onClick={resetTraffic}><RotateCcw size={14} /> RESET</button></div><div className="scenario-controls"><span>Scenario</span>{Object.entries({ offpeak: 'OFF-PEAK', normal: 'NORMAL', rush: 'RUSH HOUR', surge: 'SUDDEN SURGE' }).map(([key, label]) => <button key={key} className={scenario === key ? 'selected' : ''} onClick={() => setScenario(key)}>{label}</button>)}<div className="seed-input"><label>Seed</label><input type="number" value={simulationSeed} onChange={e => setSimulationSeed(Number(e.target.value) || 2026)} title="Same seed + same scenario = reproducible traffic" /></div></div></div>;
}
...
export default function TrafficAnalysis() {
  const { trafficState, scenario, setScenario, isRunning, startSimulation, pauseSimulation, resetTraffic, updateManualCount, setMode, mode, applyVideoAnalysis, simulationSeed, setSimulationSeed } = useTraffic();
